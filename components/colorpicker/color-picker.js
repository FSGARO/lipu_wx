Component({
  ...
  methods: {
    ...
    _chooseColor(e) {
      ...
      let x = ((e.changedTouches[0].pageX - this.data.left - this.data.scrollLeft) / this.data.rpxRatio)+140
      let y = ((e.changedTouches[0].pageY - this.data.top - this.data.scrollTop) / this.data.rpxRatio) + 140
      x = x > 480 ? 480 : x
      y = y > 480 ? 480 : y
      x = x < 0 ? 0 : x
      y = y < 0 ? 0 : y
    ...
    getColor(){
      this.triggerEvent('selectColor', {
        colorData: this.data.pickerData.hex
      })
    },
    _rgbToHex(r, g, b) {
